...
function parseRetryAfter(headers, now2 = Date.now()) {
  const raw = headers["retry-after"] ?? headers["Retry-After"];
  if (!raw) return void 0;
  const secs = Number(raw);
  if (Number.isFinite(secs)) return Math.max(0, secs * 1e3);
  const date = Date.parse(raw);
  if (Number.isFinite(date)) return Math.max(0, date - now2);
  return void 0;
}
...
async function apiCall(http, getToken, retry, { method, url, body }) {
  ...
  const res = await withRetry(
    () => http({
      ...
      contentType: body !== void 0 ? "application/json" : void 0,
...
function base64url(bytes) {
  ...
  for (; i + 2 < bytes.length; i += 3) {
    const n2 = bytes[i] << 16 | bytes[i + 1] << 8 | bytes[i + 2];
    out += table[n2 >> 18 & 63];
    out += table[n2 >> 12 & 63];
    out += table[n2 >> 6 & 63];
    out += table[n2 & 63];
...
async function postToken(http, params, retry) {
  const res = await withRetry(
    () => http({
      url: TOKEN_URL,
      method: "POST",
...
async function exchangeCode(http, config, code, verifier, now2 = Date.now, retry = {}) {
  ...
  return toTokenSet(json, now2());
}
async function refreshAccessToken(http, config, refreshToken, now2 = Date.now, retry = {}) {
  const json = await postToken(
    http,
    {
  ...
  return toTokenSet(json, now2(), refreshToken);
}
var EXPIRY_SKEW_MS = 6e4;
var GoogleAuth = class {
  constructor(http, config, store, now2 = Date.now, retry = {}) {
    this.http = http;
    this.config = config;
    this.store = store;
    this.now = now2;
...
function addQuery(url, params) {
  const query = Object.entries(params).filter((entry) => entry[1] !== void 0).map(([key, value]) => `${enc(key)}=${enc(String(value))}`).join("&");
  return query ? `${url}?${query}` : url;
}
var GoogleCalendarClient = class {
  constructor(http, getToken, retry = {}) {
    this.http = http;
  ...
  async insertEvent(calendarId, event, options = {}) {
  ...
  async patchEvent(calendarId, eventId, patch, options = {}) {
    return await this.call({
      method: "PATCH",
      url: addQuery(
  ...
  async listCalendars() {
    const r = await this.call({ method: "GET", url: `${BASE}/users/me/calendarList` });
    return r.items ?? [];
  }
  async listEvents(calendarId, options = {}) {
    ...
    do {
      ...
      const r = await this.call({ method: "GET", url });
      items.push(...r.items ?? []);
...
// src/google/tasks.ts
var BASE2 = "https://tasks.googleapis.com/tasks/v1";
var enc2 = encodeURIComponent;
function addQuery2(url, params) {
  const query = Object.entries(params).filter((entry) => entry[1] !== void 0).map(([key, value]) => `${enc2(key)}=${enc2(String(value))}`).join("&");
  return query ? `${url}?${query}` : url;
}
var GoogleTasksClient = class {
  constructor(http, getToken, retry = {}) {
    this.http = http;
    this.getToken = getToken;
    this.retry = retry;
  ...
  async listTaskLists() {
    const r = await this.call({ method: "GET", url: `${BASE2}/users/@me/lists` });
    return r.items ?? [];
  }
  async listTasks(taskListId, options = {}) {
    ...
    let pageToken;
    do {
      const url = addQuery2(`${BASE2}/lists/${enc2(taskListId)}/tasks`, {
        showCompleted: true,
        showHidden: true,
        maxResults: options.pageSize,
        pageToken
   ...
   */
  async insertTask(taskListId, task, options = {}) {
    return await this.call({
      method: "POST",
      url: addQuery2(`${BASE2}/lists/${enc2(taskListId)}/tasks`, {
   ...
   */
  async moveTask(taskListId, taskId, options = {}) {
    return await this.call({
      method: "POST",
      url: addQuery2(`${BASE2}/lists/${enc2(taskListId)}/tasks/${enc2(taskId)}/move`, {
        parent: options.parent,
        previous: options.previous
      })
    });
  ...
  async getTask(taskListId, taskId) {
    return await this.call({
      method: "GET",
      url: `${BASE2}/lists/${enc2(taskListId)}/tasks/${enc2(taskId)}`
    });
  }
  async patchTask(taskListId, taskId, patch) {
    return await this.call({
      method: "PATCH",
      url: `${BASE2}/lists/${enc2(taskListId)}/tasks/${enc2(taskId)}`,
      body: patch
...
function eventDateTime(localIso, zone, allDay = false) {
  const dt = parse2(localIso, zone);
  if (allDay) {
...
function allDayEnd(localIso, zone) {
  const date = parse2(localIso, zone).plus({ days: 1 }).toISODate();
  if (!date) throw new DateParseError(`Invalid all-day date "${localIso}"`);
  return { date };
}
function taskDue(localIso, zone) {
...
function attendeesToGoogle(attendees) {
  if (!attendees) return [];
  if (Array.isArray(attendees)) {
    return attendees.filter((a) => !!a && typeof a.email === "string" && a.email !== "").map((a) => {
      const out2 = { email: a.email };
      if (a.optional != null) out2.optional = a.optional;
      for (const k of ATTENDEE_EXTRA_KEYS) {
        if (a[k] != null) out2[k] = a[k];
      }
      return out2;
    });
  }
...
function eventToGoogle(fm, defaultTz) {
  const zone = fm.timezone || defaultTz;
  const ev = { summary: fm.title };
  ...
  if (fm.recurrence) {
    const lines = (Array.isArray(fm.recurrence) ? fm.recurrence : [fm.recurrence]).filter(
      (l2) => typeof l2 === "string" && l2.trim() !== ""
    );
    if (lines.length) ev.recurrence = lines;
  }
  return ev;
}
function taskToGoogle(fm, defaultTz) {
  const task = { title: fm.title };
...
function requireTitle(fm, errors) {
  if (typeof fm.title !== "string" || fm.title.trim() === "") {
    errors.push("`title` is required");
  }
}
function validateEvent(fm) {
  const errors = [];
  ...
  return { ok: true, value: fm, errors: [] };
}
function validateTask(fm) {
  const errors = [];
  requireTitle(fm, errors);
  ...
  if (fm.parent != null && typeof fm.parent !== "string")
...
// headless/cli.ts
var USAGE = `usage: google --config <gsync.json> <group> <command> [id] [flags]
...
async function readJsonInput(flags) {
  let raw = flags.json;
  if (raw === "-" || raw === void 0) {
    raw = await new Promise((resolve2, reject) => {
      let data = "";
      import_node_process.default.stdin.setEncoding("utf8");
      import_node_process.default.stdin.on("data", (c) => data += c);
      import_node_process.default.stdin.on("end", () => resolve2(data));
...
function eventBody(input, config, requireComplete) {
  if (requireComplete) {
...
function taskBody(input, config, requireComplete) {
  if (requireComplete) {
    const v = validateTask(input);
    if (!v.ok) fail(`invalid task: ${v.errors.join("; ")}`);
  }
  const body = taskToGoogle(input, config.settings.defaultTimezone);
...
async function main() {
  ...
  const writeOpts = flags.sendUpdates ? { sendUpdates: flags.sendUpdates } : {};
  ...
  switch (key) {
    ...
    case "events create": {
      ...
      const { body, opts } = flags.raw ? { body: input, opts: {} } : eventBody(input, config, true);
    ...
    case "events update": {
      ...
      const input = await readJsonInput(flags);
      const { body, opts } = flags.raw ? { body: input, opts: {} } : eventBody(input, config, false);
    ...
    case "tasks create": {
      const input = await readJsonInput(flags);
      const body = flags.raw ? input : taskBody(input, config, true);
      output(
    ...
    case "tasks update": {
      ...
      const input = await readJsonInput(flags);
      const body = flags.raw ? input : taskBody(input, config, false);
