    ...
    function resolveBlockMap({ composeNode, composeEmptyNode }, ctx, bm, onError, tag) {
      ...
      for (const collItem of bm.items) {
        ...
        if (valueProps.found) {
          ...
          const valueNode = value ? composeNode(ctx, value, valueProps, onError) : composeEmptyNode(ctx, offset2, sep2, null, valueProps, onError);
          if (ctx.schema.compat)
...
function isPast(localIso, zone, now2 = DateTime.now()) {
  const dt = DateTime.fromISO(localIso, { zone });
  return dt.isValid && dt < now2;
}

// src/sync/frontmatter.ts
function trimSlashes(s2) {
  return s2.replace(/^\/+|\/+$/g, "");
}
function detectKind(path, eventsFolder, tasksFolder) {
...
var MANAGED_SUBFOLDERS = ["archive", "overdue", "completed", "orphaned"];
function isManagedSubpath(path, eventsFolder, tasksFolder) {
  const p = trimSlashes(path);
...
function attendeeHasMetadata(a) {
...
function remoteEventToNote(event, calendarId) {
  ...
  const recur = event.recurrence?.filter((l2) => typeof l2 === "string" && l2.trim() !== "") ?? [];
...
function mergeManagedFrontmatter(existing, incoming, kind) {
  const managed = kind === "event" ? EVENT_MANAGED_KEYS : TASK_MANAGED_KEYS;
  const preserved = {};
...
function remoteTaskToNote(task, taskListId, parentBasename) {
  const fm = {
...
// src/sync/lifecycle-plan.ts
function dest(folder, sub, basename) {
  return `${folder.replace(/\/+$/, "")}/${sub}/${basename}.md`;
}
function linkToBasename(raw) {
  let s2 = raw.trim();
  const m = s2.match(/^\[\[(.+?)\]\]$/);
  if (m && m[1]) s2 = m[1];
  s2 = (s2.split("|")[0] ?? "").split("#")[0]?.trim() ?? "";
  const seg = s2.split("/").pop() ?? s2;
  return seg.replace(/\.md$/i, "").trim();
}
function planLifecycle(notes, cfg, now2) {
  const actions = [];
  for (const note of notes) {
    if (note.kind === "event") {
      ...
      const zone = typeof note.fm.timezone === "string" && note.fm.timezone || cfg.defaultTimezone;
      const threshold = DateTime.fromISO(date, { zone }).plus({
        days: cfg.autoArchiveDaysPast
      });
      if (!threshold.isValid || now2 <= threshold) continue;
      const linked = note.fm.tasks;
      const closeTasks = cfg.autoCloseTasksOnArchive && Array.isArray(linked) ? linked.filter((t) => typeof t === "string").map(linkToBasename).filter((t) => t.length > 0) : [];
      actions.push({
        path: note.path,
        type: "archive",
      ...
      } else if (typeof note.fm.due === "string" && isPast(note.fm.due, cfg.defaultTimezone, now2)) {
        actions.push({
          path: note.path,
          type: "overdue",
...
var TASK_BODY_KEYS = ["title", "notes", "due", "status", "parent"];
function projectRemoteBody(remote, kind) {
  const keys = kind === "event" ? EVENT_BODY_KEYS : TASK_BODY_KEYS;
  const out = {};
  for (const k of keys) {
    if (remote[k] !== void 0) out[k] = remote[k];
  }
  return out;
}
...
function diffBody(baseline, current) {
  const patch = {};
  for (const [k, v] of Object.entries(current)) {
    if (v === void 0) continue;
    const same = k === "start" || k === "end" ? sameEventTime(baseline[k], v) : sameValue(baseline[k], v);
    if (!same) patch[k] = v;
  }
  for (const k of Object.keys(baseline)) {
    if (baseline[k] === void 0 || current[k] !== void 0) continue;
    if (k === "start" || k === "end") continue;
    patch[k] = null;
  }
  return Object.keys(patch).length ? patch : null;
}
var PLACEHOLDER_TITLES = /* @__PURE__ */ new Set(["untitled event", "untitled task", "event title", "untitled"]);
function isPlaceholderTitle(value) {
  return typeof value === "string" && (value.trim() === "" || PLACEHOLDER_TITLES.has(value.trim().toLowerCase()));
}
function vetPatch(patch, baseline, kind) {
...
var SyncRouter = class {
  ...
  syncKind(path) {
    ...
    if (isManagedSubpath(path, s2.eventsFolder, s2.tasksFolder)) return null;
    return detectKind(path, s2.eventsFolder, s2.tasksFolder);
  }
  ...
  async syncAll(options = {}) {
    const result = { synced: 0, failed: 0, blocked: [] };
    const plans = [];
    for (const ref of await this.port.listMarkdown(this.scopeRoots())) {
      ...
      try {
        ...
        result.failed++;
        this.notify(`google-sync: ${noteName(ref.path)}: ${e.message}`);
      }
    }
    ...
    for (const plan of plans) {
      try {
        ...
        result.failed++;
        this.notify(`google-sync: ${noteName(plan.path)}: ${e.message}`);
      }
    }
    return result;
  }
  ...
  async previewAll() {
    ...
    for (const ref of await this.port.listMarkdown(this.scopeRoots())) {
      ...
      try {
        ...
        out.push({ path: ref.path, changedKeys: [], veto: e.message });
      }
    }
    return out;
  }
  scopeRoots() {
    const s2 = this.settings();
    return [s2.eventsFolder, s2.tasksFolder];
  }
  maxPatchesPerRun() {
    const n2 = this.settings().maxPatchesPerRun;
    return Number.isFinite(n2) && n2 > 0 ? n2 : 10;
  }
  /**
  ...
  async planPath(path, options = {}) {
    ...
    if (!patch) {
      const wantsMeet = plan.event && (plan.event.conferencing === true || plan.event.conferencing === "hangoutsMeet") && !plan.event.meetLink;
  ...
  async fetchRemoteBaseline(plan) {
    const remote = plan.kind === "event" ? await this.calendar.getEvent(plan.container, plan.googleId) : await this.tasks.getTask(plan.container, plan.googleId);
    return projectRemoteBody(remote, plan.kind);
  }
  async planEvent(path, fm) {
    ...
    const s2 = this.settings();
    const body = eventToGoogle(v.value, s2.defaultTimezone);
    return {
      path,
      kind: "event",
      container: v.value.calendarId || s2.defaultCalendarId,
  ...
  async planTask(path, fm) {
    ...
    if (!v.value.googleId) return null;
    const s2 = this.settings();
    const taskListId = v.value.tasklist || s2.taskListId;
    if (!taskListId) {
      this.notify("google-sync: set a task list ID in settings before syncing tasks.");
      return null;
    }
    const body = taskToGoogle(v.value, s2.defaultTimezone);
  ...
  async applyEvent(plan) {
    const value = plan.event;
  ...
  async writeMeetLinkBack(path, value, result) {
    const link = result.hangoutLink ?? result.conferenceData?.entryPoints?.find((e) => e.entryPointType === "video")?.uri;
   ...
   */
  async resolveParentGoogleId(parent) {
    if (typeof parent !== "string" || parent.trim() === "") return void 0;
    const target = linkToBasename(parent);
    if (!target) return void 0;
    const s2 = this.settings();
    for (const ref of await this.port.listMarkdown([s2.tasksFolder])) {
      if (ref.basename !== target) continue;
      const gid = (await this.port.readFrontmatter(ref.path)).googleId;
...
// src/vault/unused-path.ts
async function unusedPath(port, preferredPath) {
...
function patternToRegExp(pattern) {
  const body = pattern.trim().replace(/[.+?^${}()|[\]\\]/g, "\\$&").replace(/\*/g, ".*");
  return new RegExp(`^${body}$`, "i");
}
function isEventAllowed(event, mode, filters) {
...
// src/sync/orphans.ts
function emptySeen(timeMin = "", timeMax = "") {
  return {
    eventIds: /* @__PURE__ */ new Set(),
    cancelledEventIds: /* @__PURE__ */ new Set(),
...
var OrphanScanner = class {
  constructor(port, calendar, tasks, settings) {
    this.port = port;
    this.calendar = calendar;
    this.tasks = tasks;
    this.settings = settings;
  ...
  async scan(seen) {
    const s2 = this.settings();
    let moved = 0;
    for (const ref of await this.port.listMarkdown([s2.eventsFolder, s2.tasksFolder])) {
      try {
        if (isManagedSubpath(ref.path, s2.eventsFolder, s2.tasksFolder)) continue;
        const kind = detectKind(ref.path, s2.eventsFolder, s2.tasksFolder);
        if (!kind) continue;
        ...
        const orphaned = kind === "event" ? await this.eventOrphaned(gid, fm, seen, s2) : await this.taskOrphaned(gid, fm, seen, s2);
        if (!orphaned) continue;
        const folder = kind === "event" ? s2.eventsFolder : s2.tasksFolder;
        const dest2 = await unusedPath(this.port, `${folder}/orphaned/${ref.basename}.md`);
        await this.port.move(ref.path, dest2);
        moved++;
      } catch (e) {
  ...
  async eventOrphaned(googleId, fm, seen, s2) {
    if (seen.eventIds.has(googleId)) return false;
    if (seen.cancelledEventIds.has(googleId)) return true;
    if (!this.dateInWindow(fm, seen, s2)) return false;
    try {
      const remote = await this.calendar.getEvent(
        typeof fm.calendarId === "string" && fm.calendarId || s2.defaultCalendarId,
        googleId
      );
  ...
  async taskOrphaned(googleId, fm, seen, s2) {
    if (seen.taskIds.has(googleId)) return false;
    try {
      const remote = await this.tasks.getTask(
        typeof fm.tasklist === "string" && fm.tasklist || s2.taskListId,
        googleId
      );
  ...
  dateInWindow(fm, seen, s2) {
    if (typeof fm.date !== "string") return false;
    const zone = typeof fm.timezone === "string" && fm.timezone || s2.defaultTimezone;
...
function basePathFor(folder, id, title, fallback) {
...
async function pathFor(port, folder, id, title, fallback) {
  return unusedPath(port, basePathFor(folder, id, title, fallback));
}
async function findByGoogleId(port, folder, googleId) {
...
var GoogleImporter = class {
  constructor(port, calendar, tasks, settings, onTouch = () => {
  }, baselines) {
    this.port = port;
    this.calendar = calendar;
    this.tasks = tasks;
  ...
  async importAll(options = {}) {
    const counts = { events: 0, tasks: 0, failed: 0, orphaned: 0 };
  ...
  eventWindow() {
    ...
    const now2 = Date.now();
    return {
      timeMin: new Date(now2 - Math.max(0, s2.importPastDays) * dayMs).toISOString(),
      timeMax: new Date(now2 + Math.max(0, s2.importFutureDays) * dayMs).toISOString()
    };
  }
  async importEvents(counts, options, seen, window) {
  ...
  async calendarIds() {
    const s2 = this.settings();
    if (s2.importOnlyDefaultCalendar) return [s2.defaultCalendarId];
    const calendars = await this.calendar.listCalendars();
    const ids = calendars.map((c) => c.id).filter((id) => !!id);
    if (!ids.includes(s2.defaultCalendarId)) ids.unshift(s2.defaultCalendarId);
    return Array.from(new Set(ids));
  }
  async upsertEvent(calendarId, event, counts, options) {
    try {
      const s2 = this.settings();
      if (!isEventAllowed(event, s2.recurringEventFilterMode, s2.recurringEventFilters)) return;
      const fm = remoteEventToNote(event, calendarId);
      const existing = await findByGoogleId(this.port, s2.eventsFolder, event.id);
      if (existing) {
        ...
        await this.baselines?.set(existing, projectRemoteBody(event, "event"));
      } else {
        ...
        this.onTouch(path);
        await this.baselines?.set(path, projectRemoteBody(event, "event"));
      }
      counts.events++;
    } catch (e) {
      counts.failed++;
  ...
  async importTasks(counts, options, seen) {
    ...
    for (const taskListId of taskListIds) {
      ...
      const basenameById = /* @__PURE__ */ new Map();
      for (const task of tasks) {
        if (task.id) seen.taskIds.add(task.id);
        const parentBasename = task.parent ? basenameById.get(task.parent) : void 0;
  ...
  async taskListIds() {
    const s2 = this.settings();
    if (s2.importOnlyDefaultTaskList) return [s2.taskListId];
    const lists = await this.tasks.listTaskLists();
    const ids = lists.map((l2) => l2.id).filter((id) => !!id);
    if (s2.taskListId && !ids.includes(s2.taskListId)) ids.unshift(s2.taskListId);
    return Array.from(new Set(ids));
  }
  /** Returns the note basename (without extension) so subtasks can link to it. */
  async upsertTask(taskListId, task, counts, options, parentBasename) {
    try {
      ...
      if (existing) {
        ...
        await this.port.writeFrontmatter(existing, merged);
        this.onTouch(existing);
        await this.baselines?.set(existing, projectRemoteBody(task, "task"));
        counts.tasks++;
      ...
      await this.port.upsertMarkdown(path, fm);
      this.onTouch(path);
      await this.baselines?.set(path, projectRemoteBody(task, "task"));
      counts.tasks++;
...
var Lifecycle = class {
  constructor(port, tasks, settings, notify = () => {
  }) {
    this.port = port;
    this.tasks = tasks;
    this.settings = settings;
  ...
  async runOnce() {
    ...
    const actions = planLifecycle(notes, s2, DateTime.now());
    const counts = { archived: 0, overdue: 0, completed: 0 };
    for (const action of actions) {
      try {
        if (!await this.port.exists(action.path)) continue;
        if (action.type === "archive" && action.closeTasks.length && s2.taskListId) {
          for (const basename of action.closeTasks)
            await this.closeLinkedTask(basename, s2);
        }
  ...
  async closeLinkedTask(basename, s2) {
    for (const ref of await this.port.listMarkdown([s2.tasksFolder])) {
      if (ref.basename !== basename) continue;
      if (detectKind(ref.path, s2.eventsFolder, s2.tasksFolder) !== "task") continue;
      const fm = await this.port.readFrontmatter(ref.path);
      const gid = fm.googleId;
      if (typeof gid === "string" && gid) {
        try {
          await this.tasks.patchTask(s2.taskListId, gid, { status: "completed" });
        } catch (e) {
          this.notify(
            `google-sync: could not close task ${basename}: ${e.message}`
          );
        }
      }
    }
...
// headless/node-vault.ts
var import_node_fs = require("node:fs");
var nodePath = __toESM(require("node:path"));
var import_yaml = __toESM(require_dist());
var FRONTMATTER_RE = /^---\n([\s\S]*?)\n---/;
var SKIP_DIRS = /* @__PURE__ */ new Set([".obsidian", ".git", ".google-sync", ".trash"]);
var NodeVaultPort = class {
  ...
  async listMarkdown(roots) {
    const out = [];
    const seen = /* @__PURE__ */ new Set();
    const walk = async (relDir) => {
      let entries;
      try {
        entries = await import_node_fs.promises.readdir(this.abs(relDir), { withFileTypes: true });
      } catch {
        return;
      }
  ...
  async exists(path) {
    try {
      await import_node_fs.promises.access(this.abs(path));
      return true;
    } catch {
      return false;
    }
  }
  async readParts(path) {
    const content = await import_node_fs.promises.readFile(this.abs(path), "utf8");
    const match2 = content.match(FRONTMATTER_RE);
  ...
  async write(path, frontmatter, body) {
    const abs = this.abs(path);
    await import_node_fs.promises.mkdir(nodePath.dirname(abs), { recursive: true });
    await import_node_fs.promises.writeFile(abs, `---
${(0, import_yaml.stringify)(frontmatter)}---
${body}`, "utf8");
  }
  async readFrontmatter(path) {
    return (await this.readParts(path)).fm;
  }
  async writeFrontmatter(path, frontmatter) {
    const { body } = await this.readParts(path);
    await this.write(path, frontmatter, body);
  }
  async writeFrontmatterKey(path, key, value) {
    const { fm, body } = await this.readParts(path);
    fm[key] = value;
    await this.write(path, fm, body);
  }
  async upsertMarkdown(path, frontmatter, body = "") {
    await this.write(path, frontmatter, body);
  }
  async move(oldPath, newPath) {
    const to = this.abs(newPath);
    await import_node_fs.promises.mkdir(nodePath.dirname(to), { recursive: true });
    await import_node_fs.promises.rename(this.abs(oldPath), to);
  }
};
...
var FileTokenStore = class {
  ...
  async load() {
    try {
      const raw = await import_node_fs2.promises.readFile(this.file, "utf8");
  ...
  async save(tokens) {
    if (!tokens) {
      await import_node_fs2.promises.unlink(this.file).catch(() => void 0);
      return;
    }
    await import_node_fs2.promises.mkdir(nodePath2.dirname(this.file), { recursive: true });
    const tmp = `${this.file}.tmp`;
    await import_node_fs2.promises.writeFile(tmp, JSON.stringify(tokens, null, 2), { mode: 384 });
    await import_node_fs2.promises.rename(tmp, this.file);
  }
};
...
var FileBaselineStore = class {
  ...
  get file() {
    return nodePath3.join(this.vaultPath, ".google-sync", "state.json");
  }
  async load() {
    if (this.state) return this.state;
    try {
      const raw = JSON.parse(await import_node_fs3.promises.readFile(this.file, "utf8"));
  ...
  async flush() {
    ...
    const state = await this.load();
    await import_node_fs3.promises.mkdir(nodePath3.dirname(this.file), { recursive: true });
    await import_node_fs3.promises.writeFile(this.file, JSON.stringify(state, null, 2) + "\n", "utf8");
    this.dirty = false;
  }
};
...
var GitSync = class {
  constructor(vaultPath, cfg, log2 = () => {
  }, runner) {
    this.vaultPath = vaultPath;
    this.cfg = cfg;
    this.log = log2;
    this.run = runner ?? (async (args) => {
  ...
  async preSync() {
    ...
    try {
      ...
      this.log(`git: fetch failed (${e.message.split("\n")[0]}); continuing offline`);
      return;
    }
  ...
  async commitAndPush(paths, message) {
    ...
    for (let attempt = 1; ; attempt++) {
      try {
        ...
        if (attempt >= 3) {
          throw new Error(
            `git push failed after ${attempt} attempts: ${e.message.split("\n")[0]}`
          );
        }
...
async function loadConfig(file) {
  const configDir = nodePath4.dirname(nodePath4.resolve(file));
  let raw;
  try {
    raw = JSON.parse(await import_node_fs4.promises.readFile(file, "utf8"));
  } catch (e) {
  ...
  const vaultPath = nodePath4.resolve(configDir, raw.vaultPath);
  const tokenFile = nodePath4.resolve(
    configDir,
    typeof raw.tokenFile === "string" && raw.tokenFile ? raw.tokenFile : "gsync-tokens.json"
  );
  if (tokenFile.startsWith(vaultPath + nodePath4.sep)) {
    throw new Error(
...
function parseArgs(argv) {
  const args = { config: "", dryRun: false, allowMassUpdate: false, noGit: false };
...
var log = (msg) => console.log(`[gsync ${(/* @__PURE__ */ new Date()).toISOString()}] ${msg}`);
var LOCK_STALE_MS = 30 * 60 * 1e3;
async function acquireLock(vaultPath) {
  const key = (0, import_node_crypto.createHash)("sha1").update(vaultPath).digest("hex").slice(0, 12);
  const lockFile = nodePath5.join(os.tmpdir(), `gsync-${key}.lock`);
  try {
    const raw = await import_node_fs5.promises.readFile(lockFile, "utf8");
    const { time } = JSON.parse(raw);
    if (time && Date.now() - time < LOCK_STALE_MS) return null;
    log("stale lock found \u2014 taking over");
  } catch {
  }
  await import_node_fs5.promises.writeFile(lockFile, JSON.stringify({ pid: import_node_process.default.pid, time: Date.now() }));
  return async () => import_node_fs5.promises.unlink(lockFile).catch(() => void 0);
}
async function runSync(config, args) {
  ...
  const importer = new GoogleImporter(port, calendar, tasks, settings, () => {
  }, baselines);
  ...
  const live = new Set(
    (await port.listMarkdown([config.settings.eventsFolder, config.settings.tasksFolder])).map((r) => r.path)
  );
...
async function main() {
  ...
  try {
    ...
    if (gitEnabled) {
      const s2 = config.settings;
      const stamp = (/* @__PURE__ */ new Date()).toISOString();
      await git.commitAndPush(
        [s2.eventsFolder, s2.tasksFolder, ".google-sync"],
        `google-sync: ${stamp}`
      );
      log("git: pushed");
    }
...
main().then((code) => import_node_process.default.exit(code)).catch((e) => {
  console.error(`[gsync] fatal: ${e.stack ?? String(e)}`);
